...
export const SNSPosts: React.FC = () => {
  ...
  const followerStats = useMemo(() => {
    ...
    const byWeek = Array.from(weekMap.entries())
      .map(([week, { count, weekStart: ws }]) => ({
        week,
        label: format(ws, 'M/d', { locale: ja }),
        count,
      }))
  ...
  const historicalPosts = useMemo(() => {
    if (!allPosts) return [];
    return allPosts.filter(p => p.postedAt).sort((a, b) => new Date(b.postedAt).getTime() - new Date(a.postedAt).getTime());
  }, [allPosts]);
  ...
  const availableMonths = useMemo(() => {
    ...
    allPosts.forEach(post => {
      if (post.postedAt) months.add(format(new Date(post.postedAt), 'yyyy-MM'));
    });
    const now = new Date();
    ...
    for (let month = 4; month <= 12; month++) months.add(`${fiscalYearStart}-${String(month).padStart(2, '0')}`);
    for (let month = 1; month <= 3; month++) months.add(`${fiscalYearEnd}-${String(month).padStart(2, '0')}`);
    const prevFiscalYearStart = fiscalYearStart - 1;
    for (let month = 1; month <= 3; month++) months.add(`${prevFiscalYearStart + 1}-${String(month).padStart(2, '0')}`);
                    ...
                    {currentWeekStatus.map((status) => (
                      <tr key={status.userId} className="border-b border-gray-200 dark:border-gray-700">
                        ...
                        <td className="py-3 px-4 text-center">
                          {status.hasFeed
                            ? <span className="text-green-600 dark:text-green-400 font-medium">✓ {status.feedPosts.length > 1 ? `${status.feedPosts.length}件` : '投稿済み'}</span>
                            : <span className="text-red-600 dark:text-red-400 font-medium">未投稿</span>}
                        </td>
                        <td className="py-3 px-4 text-center">
                          {status.hasStory
                            ? <span className="text-green-600 dark:text-green-400 font-medium">✓ {status.storyPosts.length > 1 ? `${status.storyPosts.length}件` : '投稿済み'}</span>
                            : <span className="text-red-600 dark:text-red-400 font-medium">未投稿</span>}
                        </td>
                      </tr>
                    ))}
                ...
                {historicalPosts.map((post) => (
                  <div key={post.id} className="bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-xs px-2 py-1 rounded-full ${post.postType === 'STORY' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300'}`}>
                        {post.postType === 'STORY' ? 'ストーリーズ' : 'フィード'}
                      </span>
                      {post.user && <span className="text-xs text-gray-500 dark:text-gray-400">({post.user.name})</span>}
                      {post.url && (
                        <a href={post.url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 dark:text-blue-400 flex items-center gap-1">
                          <ExternalLink className="w-3 h-3" />投稿リンク
                        </a>
                      )}
                    </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-border dark:border-gray-700 overflow-hidden">
            <div className="px-4 py-3 border-b border-border dark:border-gray-700">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                各週のストーリーズ／フィードの記録状況です。セルを押して記録・編集できます。
              </p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[520px]">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40">
                    ...
                    <th className="text-center py-3 px-2 font-semibold text-purple-700 dark:text-purple-300">ストーリーズ</th>
                    <th className="text-center py-3 px-2 font-semibold text-blue-700 dark:text-blue-300">フィード</th>
                  </tr>
                </thead>
                <tbody>
                  {personalWeekRows.map((row) => {
                    ...
                    const renderPost = (p: SNSPost, type: 'STORY' | 'FEED') => (
                      <div key={p.id} className="flex items-stretch gap-1 w-full">
                        <button
                          type="button"
                          className={`flex-1 rounded-l-md border text-xs px-2 py-1.5 transition-colors text-left ${
                            type === 'STORY'
                              ? 'border-purple-200 dark:border-purple-900/50 bg-purple-50/50 dark:bg-purple-900/20 hover:bg-purple-100 dark:hover:bg-purple-900/40'
                              : 'border-blue-200 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/40'
                          }`}
                          onClick={() => setEditingPost(p)}
                        >
                          <span className={`font-medium ${type === 'STORY' ? 'text-purple-600 dark:text-purple-400' : 'text-blue-600 dark:text-blue-400'}`}>
                        ...
                        <button
                          ...
                          className={`px-1.5 rounded-r-md border-y border-r text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 flex-shrink-0 ${
                            type === 'STORY'
                      ...
                      <tr key={row.weekKey} className="border-b border-gray-100 dark:border-gray-700/80">
                        ...
                        <td className="py-2 px-2">
                          <div className="flex flex-col gap-1">
                            {sPosts.length > 0 ? sPosts.map(p => renderPost(p, 'STORY')) : renderEmpty('STORY')}
                          </div>
                        </td>
                        <td className="py-2 px-2">
                          <div className="flex flex-col gap-1">
                            {fPosts.length > 0 ? fPosts.map(p => renderPost(p, 'FEED')) : renderEmpty('FEED')}
                          </div>
                        </td>
                      </tr>
                    );
            ...
            >
              <span className="font-semibold text-gray-900 dark:text-gray-100">
                ...
                {posts && posts.length > 0 && (
                  <span className="ml-2 text-xs font-normal text-gray-500 dark:text-gray-400">{posts.length}件</span>
                )}
              </span>
                        ...
                        <div key={post.id} className="bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <span className={`text-xs px-2 py-0.5 rounded-full ${post.postType === 'STORY' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300'}`}>
                                {post.postType === 'STORY' ? 'ストーリーズ' : 'フィード'}
                              </span>
                              {post.followerCount != null && (
                                <span className="text-xs text-gray-500 dark:text-gray-400">{post.followerCount.toLocaleString()}人</span>
                              )}
                              {post.url && (
                                <a href={post.url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 dark:text-blue-400 flex items-center gap-1">
