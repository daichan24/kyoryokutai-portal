...
export const ProfileSettings: React.FC = () => {
  ...
  const [contactsSidebarEnabled, setContactsSidebarEnabled] = useState(false);
  ...
  useEffect(() => {
    if (user) {
      ...
      setContactsSidebarEnabled(user.contactsSidebarEnabled === true);
  ...
  const handleSaveProfile = () => {
    profileMutation.mutate({
      ...
      contactsSidebarEnabled: contactsSidebarEnabled,
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">サイドバーに町民データベースを表示する</p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                checked={contactsSidebarEnabled}
                onChange={(e) => setContactsSidebarEnabled(e.target.checked)}
                className="sr-only peer"
              />
              <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-600 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-primary"></div>
              <span className="ms-3 text-sm text-gray-600 dark:text-gray-400">{contactsSidebarEnabled ? 'ON' : 'OFF'}</span>
            </label>
          </div>
